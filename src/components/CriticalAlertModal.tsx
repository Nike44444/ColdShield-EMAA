...
export function CriticalAlertModal({ alerts, sensors, onAcknowledge }: CriticalAlertModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-slate-950/80 p-4 backdrop-blur-sm sm:items-center">
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-red-400/60 bg-slate-950 shadow-[0_0_70px_rgba(239,68,68,0.35)]">
        <div className="relative overflow-hidden bg-gradient-to-r from-red-700 via-red-600 to-rose-700 p-5">
          <div className="absolute -right-5 -top-5 h-24 w-24 rounded-full bg-white/10 blur-xl" />
          <div className="relative flex items-start gap-3">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white/15 text-white animate-pulse">
              <BellRing className="h-6 w-6" />
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-red-100">EMAA critical alert</p>
              <h2 className="mt-1 text-xl font-bold text-white">Temperature breach — hold vaccine</h2>
            </div>
          </div>
        </div>
        <div className="p-5">
          <div className="flex gap-3 rounded-2xl border border-red-900/70 bg-red-950/35 p-4">
            <AlertOctagon className="h-5 w-5 shrink-0 text-red-400" />
            <div>
              <p className="font-semibold text-red-100">Pharmacist review required</p>
              <p className="mt-1 text-sm text-slate-300">{critical.message}</p>
            </div>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-xl bg-slate-900 p-3"><p className="text-xs text-slate-500">Storage unit</p><p className="mt-1 font-medium text-white">{sensor?.name ?? 'Unknown unit'}</p></div>
            <div className="rounded-xl bg-slate-900 p-3"><p className="flex items-center gap-1 text-xs text-slate-500"><MapPin className="h-3 w-3" /> Location</p><p className="mt-1 truncate font-medium text-white">{sensor?.location ?? 'Not available'}</p></div>
          </div>
